...
internal class UploadEntryAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val holder: Holder
        if (convertView == null) {
            view = inflater.inflate(R.layout.row_upload, parent, false)
            holder = Holder(
                icon = view.findViewById(R.id.upload_icon),
                name = view.findViewById(R.id.upload_name),
                meta = view.findViewById(R.id.upload_meta),
                open = view.findViewById(R.id.upload_open),
                delete = view.findViewById(R.id.upload_delete)
            )
            view.tag = holder
        } else {
            view = convertView
        ...
        holder.open.setText(if (apk) R.string.install else R.string.open)
        if (apk) {
            holder.open.setBackgroundResource(R.drawable.button_green_selector)
        } else {
            holder.open.setBackgroundResource(R.drawable.button_primary_selector)
        }
        holder.open.setOnClickListener { listener.onOpen(file) }
        holder.delete.setOnClickListener { listener.onDelete(file) }
        return view
    }
