...
import static com.gothwad.filemanager.misc.DiskInfo.FLAG_SD;
import static com.gothwad.filemanager.misc.DiskInfo.FLAG_USB;
...
public final class StorageUtils {
    ...
    public List<StorageVolume> getStorageMounts() {
        ...
        // 1. Try official StorageManager API (API 24+)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && mStorageManager != null) {
            try {
                List<android.os.storage.StorageVolume> volumes = mStorageManager.getStorageVolumes();
                if (volumes != null && !volumes.isEmpty()) {
                    for (android.os.storage.StorageVolume sv : volumes) {
                        File dir = null;
                        ...
                        String desc = "";
                        try {
                            desc = sv.getDescription(mContext);
                        } catch (Throwable ignored) {}
                        if (TextUtils.isEmpty(desc)) {
                            desc = sv.isPrimary() ? (mContext != null ? mContext.getString(R.string.root_internal_storage) : "Internal Storage") : "Storage";
                        }
                        boolean isPrimary = sv.isPrimary();
                        ...
                        StorageVolume storageVolume = new StorageVolume(
                                isPrimary ? StorageVolume.STORAGE_ID_PRIMARY : StorageVolume.STORAGE_ID_INVALID,
                                dir, desc, isPrimary, isRemovable, isEmulated, 0, false, 0);
                        storageVolume.mUuid = uuid;
                        storageVolume.mUserLabel = desc;
                        storageVolume.mState = Environment.MEDIA_MOUNTED;
        ...
        // 2. Fallback to reflection if empty
        if (mounts.isEmpty() && mStorageManager != null) {
            try {
        ...
        // 3. Fallback: Always ensure at least primary internal storage exists
        if (mounts.isEmpty()) {
            File primaryPath = Environment.getExternalStorageDirectory();
            if (primaryPath != null) {
                StorageVolume primary = new StorageVolume(
                        StorageVolume.STORAGE_ID_PRIMARY,
                        primaryPath,
                        mContext != null ? mContext.getString(R.string.root_internal_storage) : "Internal Storage",
                        true, false, true, 0, false, 0);
                primary.mState = Environment.MEDIA_MOUNTED;
                mounts.add(primary);
            }
        }
        return mounts;
    ...
    private File getFile(Object object) {
        if (object == null) return null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && object instanceof android.os.storage.StorageVolume) {
            try {
                File dir = ((android.os.storage.StorageVolume) object).getDirectory();
                if (dir != null) return dir;
            } catch (Throwable ignored) {}
        }
        try {
            Method getPathFile = object.getClass().getDeclaredMethod("getPathFile");
            getPathFile.setAccessible(true);
            Object res = getPathFile.invoke(object);
            if (res instanceof File) return (File) res;
        } catch (Throwable ignored) {}
        try {
            Method getPath = object.getClass().getDeclaredMethod("getPath");
            getPath.setAccessible(true);
            Object res = getPath.invoke(object);
            if (res instanceof String) return new File((String) res);
            if (res instanceof File) return (File) res;
        } catch (Throwable ignored) {}
        try {
            ...
            if (pathObj instanceof File) {
                ...
                return new File((String) pathObj);
            }
        } catch (Throwable ignored) {}
        return null;
    }
    ...
    private String getDescription(Object object, boolean hasId) {
        ...
        if (hasId) {
            ...
            try {
                if (mContext != null && mContext.getResources() != null && mDescriptionInt != 0) {
