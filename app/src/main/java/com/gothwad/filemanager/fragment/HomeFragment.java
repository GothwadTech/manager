...
import com.gothwad.filemanager.adapter.CommonInfo;
import com.gothwad.filemanager.adapter.ConnectionsAdapter;
import com.gothwad.filemanager.adapter.HomeAdapter;
import com.gothwad.filemanager.adapter.RecentsAdapter;
import com.gothwad.filemanager.common.DialogBuilder;
import com.gothwad.filemanager.common.RecyclerFragment;
import com.gothwad.filemanager.cursor.LimitCursorWrapper;
...
import static com.gothwad.filemanager.adapter.HomeAdapter.TYPE_MAIN;
import static com.gothwad.filemanager.adapter.HomeAdapter.TYPE_RECENT;
import static com.gothwad.filemanager.adapter.HomeAdapter.TYPE_SHORTCUT;
import static com.gothwad.filemanager.misc.AnalyticsManager.FILE_TYPE;
import static com.gothwad.filemanager.provider.AppsProvider.getRunningAppProcessInfo;
...
public class HomeFragment extends RecyclerFragment implements HomeAdapter.OnItemClickListener {
    ...
    public void showData(){
        ...
        if (mIconHelper == null && mActivity != null) {
            mIconHelper = new IconHelper(mActivity, MODE_GRID);
        }
        roots = DocumentsApplication.getRootsCache(getActivity());
        if (mIconHelper != null && mActivity != null && mActivity.getDisplayState() != null) {
        ...
        getRecentsData();
        ArrayList<CommonInfo> data = new ArrayList<>();
        if (mainData != null) data.addAll(mainData);
        if (shortcutsData != null) data.addAll(shortcutsData);
        if (mAdapter != null) {
        ...
        mCallbacks = new LoaderManager.LoaderCallbacks<DirectoryResult>() {
            ...
            public void onLoadFinished(Loader<DirectoryResult> loader, DirectoryResult result) {
                if (!isAdded() || mAdapter == null)
                    return;
                if(null != result && null != result.cursor && result.cursor.getCount() != 0) {
        ...
        try {
            ...
            if (listView != null) {
                ...
                if (!(lm instanceof GridLayoutManager)) {
                    int span = totalSpanSize > 0 ? totalSpanSize : 4;
                    lm = new GridLayoutManager(getActivity(), span);
                    ...
                    public int getSpanSize(int position) {
                        ...
                        if (mAdapter != null && position >= 0 && position < mAdapter.getItemCount()) {
                            ...
                            if (item != null) {
                                switch (item.type) {
                                    case TYPE_MAIN:
                                        spanSize = totalSpanSize > 0 ? totalSpanSize : 4;
                                        break;
                                    case TYPE_SHORTCUT:
