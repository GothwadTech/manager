...
import com.gothwad.filemanager.archive.DocumentArchiveHelper;
import com.gothwad.filemanager.cursor.MatrixCursor;
import com.gothwad.filemanager.cursor.MatrixCursor.RowBuilder;
import com.gothwad.filemanager.libcore.io.IoUtils;
import com.gothwad.filemanager.misc.CrashReportingManager;
import com.gothwad.filemanager.misc.DiskInfo;
import com.gothwad.filemanager.misc.FileUtils;
import com.gothwad.filemanager.misc.MimePredicate;
import com.gothwad.filemanager.misc.ParcelFileDescriptorUtil;
import com.gothwad.filemanager.misc.StorageUtils;
import com.gothwad.filemanager.misc.StorageVolume;
import com.gothwad.filemanager.misc.Utils;
import com.gothwad.filemanager.misc.VolumeInfo;
...
import static com.gothwad.filemanager.misc.FileUtils.getTypeForFile;
import static com.gothwad.filemanager.model.DocumentInfo.getCursorString;
import static com.gothwad.filemanager.provider.UsbStorageProvider.ROOT_ID_USB;
...
public class ExternalStorageProvider extends StorageProvider {
    ...
    private void updateVolumesLocked() {
        ...
        for (StorageVolume storageVolume : storageUtils.getStorageMounts()) {
            ...
            try {
                state = EnvironmentCompat.getStorageState(path);
            ...
            if (storageVolume.isPrimary()) {
                ...
                title = getContext() != null ? getContext().getString(R.string.root_internal_storage) : "Internal Storage";
                ...
                title = !TextUtils.isEmpty(label) ? label
                        : (getContext() != null ? getContext().getString(R.string.root_external_storage) : "SD Card")
                        + (count > 0 ? " " + count : "");
                count++;
            } else {
                rootId = ROOT_ID_SECONDARY + count;
                title = (getContext() != null ? getContext().getString(R.string.root_external_storage) : "Storage")
                        + (count > 0 ? " " + count : "");
                count++;
            }
            ...
            try {
                ...
                Log.e(TAG, "Failed to initialize root for " + path, e);
            }
        }

        // Always guarantee at least primary internal storage is present
        if (!mRoots.containsKey(ROOT_ID_PRIMARY_EMULATED)) {
            try {
                File primaryPath = Environment.getExternalStorageDirectory();
                if (primaryPath != null) {
                    final RootInfo root = new RootInfo();
                    root.rootId = ROOT_ID_PRIMARY_EMULATED;
                    root.flags = Root.FLAG_SUPPORTS_CREATE | Root.FLAG_SUPPORTS_EDIT | Root.FLAG_LOCAL_ONLY | Root.FLAG_ADVANCED
                            | Root.FLAG_SUPPORTS_SEARCH | Root.FLAG_SUPPORTS_IS_CHILD;
                    root.title = getContext() != null ? getContext().getString(R.string.root_internal_storage) : "Internal Storage";
                    root.path = primaryPath;
                    root.docId = getDocIdForFile(primaryPath);
                    mRoots.put(ROOT_ID_PRIMARY_EMULATED, root);
                }
            } catch (Throwable e) {
                Log.e(TAG, "Failed to fallback primary root", e);
            }
        }
    }
